...
クラウドストレージを使ったバージョン管理された環境変数の管理ツール
...
**syncenv**は、Gitのブランチやタグに応じた環境変数をクラウドストレージから自動で取得・保存できるCLIツールです。暗号化にも対応し、チーム全体で安全にバージョン管理された環境変数を共有できます。

## エレベーターピッチ（30秒版）

「v1.5で動作確認したいのに、新しいメンバーが環境変数のバージョン違いで動かせない」そんな経験ありませんか?

**syncenv**は、Gitのブランチやタグに応じた環境変数を、AWS/Azure/GCPのストレージから自動で取得・保存できるCLIツールです。暗号化にも対応し、チーム全体で安全にバージョン管理された環境変数を共有できます。

`git checkout v1.5` → `syncenv pull` これだけで、そのバージョンの環境変数が手に入ります。
...
| `syncenv push [--tag TAG]` | 環境変数をアップロード |
| `syncenv pull [--tag TAG] [-f]` | 環境変数をダウンロード |
...
- **デバッグ**: 古いバージョンに簡単に切り替えて正しい環境変数で実行
...
### 問題
プロダクトがv1.5からv1.6にアップデートされると、環境変数のスキーマも変わります。本番で問題が起きてv1.5で調査したいとき、新しいメンバーは「v1.5の環境変数がわからない」「動かせない」という状況に陥ります。

### 解決策
**syncenv**は、環境変数をGitのバージョン（タグ/ブランチ）と紐付けてクラウドストレージで管理するOSSツールです。

```bash
git checkout v1.5
syncenv pull  # v1.5の環境変数を自動取得
```

### ターゲット
バージョン管理されたプロダクトを開発するチーム、特にマイクロサービスやAPI開発者

**OSSとして公開。あなたのチームの環境変数管理を変えませんか？**
